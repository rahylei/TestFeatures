...
>
    ...
    <div class="flex items-center gap-3">
        <x-kuijet.button type="button" class="md:hidden" iconOnly variant="secondary" srText="Toggle dark mode"
            @click="toggleTheme">
    ...
    <div class="flex items-center gap-3">
        <x-kuijet.button type="button" class="hidden md:inline-flex" iconOnly variant="secondary" srText="Toggle dark mode"
            @click="toggleTheme">
            <x-heroicon-o-moon x-show="!isDarkMode" aria-hidden="true" class="w-6 h-6" />
            <x-heroicon-o-sun x-show="isDarkMode" aria-hidden="true" class="w-6 h-6" />
        </x-kuijet.button>
        ...
        @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
            <x-kuijet.dropdown align="right" width="60">
                <x-slot name="trigger">
                ...
                <x-slot name="content">
                    <div class="w-60">
                        ...
                        <!-- Team Settings -->
                        <x-kuijet.dropdown-link href="{{ route('teams.show', Auth::user()->currentTeam->id) }}">
                            {{ __('Team Settings') }}
                        </x-kuijet.dropdown-link>

                        @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                            <x-kuijet.dropdown-link href="{{ route('teams.create') }}">
                                {{ __('Create New Team') }}
                            </x-kuijet.dropdown-link>
                        @endcan
                        ...
                        @foreach (Auth::user()->allTeams() as $team)
                            <x-kuijet.switchable-team :team="$team" />
                        @endforeach
                    </div>
                </x-slot>
            </x-kuijet.dropdown>
        ...
        <!-- Settings Dropdown -->
        <x-kuijet.dropdown align="right" width="48">
            <x-slot name="trigger">
            ...
            <x-slot name="content">
                ...
                <x-kuijet.dropdown-link href="{{ route('profile.show') }}">
                    {{ __('Profile') }}
                </x-kuijet.dropdown-link>

                @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                    <x-kuijet.dropdown-link href="{{ route('api-tokens.index') }}">
                        {{ __('API Tokens') }}
                    </x-kuijet.dropdown-link>
                @endif
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-kuijet.dropdown-link :href="route('logout')" onclick="event.preventDefault();
